...
app = FastAPI(title="Banknote Detector")
...
def load_model():
    """Pre-load the model at startup so any errors surface immediately."""
    try:
        get_predictor()
...
def index(request: Request):
    """Serve the main page, passing model metadata to the template."""
    try:
...
async def api_predict(file: UploadFile = File(...)):
    """Accept an image upload, run inference, and return the prediction."""
    ...
    try:
        ...
        img = ImageOps.exif_transpose(img)  # fix rotation from EXIF data

        # Create a small thumbnail for display in the UI
        thumb = img.copy()
